package com.mrslyfoxjr.arquebus.entities;
...
import com.mrslyfoxjr.arquebus.util.CommonUtils;
    ...
    protected void onHit(RayTraceResult ray)
    {
        ...
        if(entityHit != null)
        {
            ...
            if(entityHit.attackEntityFrom(damageSource, damage))
            {
                ...
                //Play hit sound
                playSound(SoundEvents.BLOCK_LAVA_POP, 1.0F, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
        ...
        {
            //Hit a block
            playSound(SoundEvents.BLOCK_ANVIL_HIT, 1.0F, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
